...
class ToDoRepository @Inject constructor (private val toDoDao: ToDoDao) {
    ...
    suspend fun getTodoBoxesWithTodosByDateAndID(selectedDate: LocalDate, boxId: Long): List<ToDoBoxWithTodos> {

        val dateStart = selectedDate.atStartOfDay()
        val dateEnd = selectedDate.atTime(LocalTime.MAX)
        val box = toDoDao.getBoxByIdAndDate(boxId,dateStart,dateEnd)

        // 检查box是否为null
        if (box == null) {
            Log.w("ToDoRepository", "No box found for the given ID and date range.")
            return emptyList() // 或者可以根据需要返回特定的错误信息或对象
        }

        val todosWithBox = run {
            val todos = toDoDao.getTodosByBoxId(boxId!!)
            val filteredTodos = todos.filter { it.status != Status.DELETED }
            val doneCount = filteredTodos.count { it.isChecked }
            ToDoBoxWithTodos(box, filteredTodos, doneCount)
        }
        Log.d("ToDoRepository", "getTodoBoxesWithTodosByDateAndID:\n$todosWithBox")
        return  listOf(todosWithBox)
    }
