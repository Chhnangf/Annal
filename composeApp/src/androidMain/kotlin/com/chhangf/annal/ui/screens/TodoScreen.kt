    ...
    Scaffold(
        ...
        content = {
                ...
                Column(
                    ...
                    BoxSelectionBar(todoViewModel, selectDate)
                    // *** 遍历数据表显示内容 *** //
...
@Composable
fun BoxSelectionBar(todoViewModel: ToDoViewModel, selectDate: LocalDate) {

    val todoBox by todoViewModel.todoBoxes.collectAsState()
    var isExtendedVisible by remember { mutableStateOf(false) }
    ...
    Row(
        ...
        todoBox.forEach { (todoBoxes, boxTitle, boxColor) ->
            // 对于"创建"按钮，我们可以单独处理样式
            val isCreateButton = boxTitle == "创建"
            val isSelected = boxTitle != "创建"
            val icon = if (isCreateButton) Icons.Default.Add else null

            BoxSelectionButton(
                title = boxTitle,
                onClick = { todoBoxes?.let { todoViewModel.onBoxSelected(selectDate, it) } },
//                isSelected = isSelected,
//                onClick = {
//                    if (isCreateButton) {
//                        // 导航到创建新Box的界面逻辑
//                        // todoViewModel.navigateToCreateBox()
//                    } else {
//                        //todoViewModel.onBoxSelected(boxTitle)
//                    }
//                },
                icon = icon
            )
...
//            onClick = { todoViewModel.navigateToCreateBox() }
        )
        // 当基础按钮加上创建按钮总数达到一定数量时，显示“更多”按钮
        if (todoBox.size + 1 >= 5) {
            IconButton(
                onClick = { isExtendedVisible = !isExtendedVisible },
                modifier = Modifier.weight(1f)
            ) {
                Icon(Icons.Default.KeyboardArrowDown, contentDescription = "Expand More Options")
            }
        }

        // 动态显示/隐藏扩展的盒子选项
        AnimatedVisibility(visible = isExtendedVisible) {
            Row(
                modifier = Modifier.fillMaxWidth(),
                horizontalArrangement = Arrangement.SpaceBetween
            ) {
                todoBox.forEach { boxes ->
                    BoxSelectionButton(
                        title = boxes.title,
                        onClick = {  },
//                        isSelected = todoViewModel.selectedBoxTitle.value == extendedBoxTitle,
//                        weight = 1f,
//                        onClick = { todoViewModel.onBoxSelected(extendedBoxTitle) }
                    )
                }
            }
        }
...
//            backgroundColor = if (isSelected) Color.LightBlue else Color.Transparent,
//            contentColor = if (isSelected) Color.White else Color.Black
        ),
        modifier = Modifier.padding(vertical = 8.dp).background(Color.Unspecified),
